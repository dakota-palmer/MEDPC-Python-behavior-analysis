...
# %% -NOTE- Things to change manually for your data:

# plot settings in section below

# %% Load previously saved dfTidy (and other vars) from pickle (generated by importMPCData.py)
...
# %% ----- Preliminary data analyses- Event latency, count, and behavioral outcome for each trial--------------

# %%- TODO: Lick 'cleaning' to eliminate invalid licks 
# (are they in port, is ILI within reasonable range?)


# %%- Calculate latency to each event in trial (from cue onset). 
# based on trialStart to keep it simple
