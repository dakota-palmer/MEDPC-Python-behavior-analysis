...
cd 'C:\Users\Dakota\Desktop\Opto DS Task Test- CUE Laser Manipulation';
...
for file= 1:numel(fileList)
    ...
    for subj= 1:numel(sheet_name)
        ...
        opts = detectImportOptions(fileList(file).name,'Sheet',cell2mat(sheet_name(subj)),'Range','A1:W10000');  %still have to specify the full range
...
%         MPCraw(all(ismissing(MPCraw),2),:)=[]; % for nan - rows
        ...
        varNames= varNames([7,8,9,10,11,12,13,14,17,20,21,22,23]); %just pulling vars i want
...
for subj= 1:numel(subjects)
    ...
    for var= 1:numel(varNames)
            if iscell(trainData(1).(varNames{var})) %if this is a cell array with string, make char
