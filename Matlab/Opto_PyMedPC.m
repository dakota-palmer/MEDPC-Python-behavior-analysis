...
cd 'Z:\Dakota\MEDPC\Downstairs\vp-vta-stgtacr_DStrain\Opto DS Task Test- CUE Laser Manipulation\_processed';
...
for file= 1:numel(fileList)
    ...
    for subj= 1:numel(sheet_name)
        ...
        opts = detectImportOptions(fileList(file).name,'Sheet',cell2mat(sheet_name(subj)),'Range','A:V');  %still have to specify the full range
...
%         MPCraw= readcell(fileList(file).name, opts);% 'Sheet', cell2mat(sheet_name(subj)));
        ...
        %!!!~~~Remove any variables you don't care about~~~~~~~~~~~~~~~!!!
        varNames= varNames([7,8,9,10,11,12,13,16,19,20,21]); %just pulling vars i want
...
ratInfo= readtable('Z:\Dakota\MEDPC\Downstairs\vp-vta-stgtacr_DStrain\vp-vta-stgtacr_subj_metadata.xlsx'); %import rat metadata from excel sheet generated manually
...
sesInfo= readtable('Z:\Dakota\MEDPC\Downstairs\vp-vta-stgtacr_DStrain\vp-vta-stgtacr_session_metadata.xlsx'); %import session metadata from excel sheet generated manually
...
for subj= 1:numel(subjects)
    ...
    for ses= sessions %for each unique session this subject ran in trainData 
        for var= 2:numel(varNames) %assume column 1 is subject
                ...
                date= str2num(trainData(ses).file(1:strfind(trainData(ses).file,'.x')-1));

                %find sessions that match this date and this subject then assign to
                %proper position in trainData
                trainData(ses).(varNames{var})= table2array(sesInfo(sesInfo.date==date & strcmp(sesInfo.subject,subjects{subj}),var));
        end
    end
end
...
for subj= 1:numel(subjects)
    ...
    for var= 1:numel(varNames)
            if iscell(trainData(1).(varNames{var})) %if this is a cell array, make char
