...
% SavingDir = 'Z:\Dakota\MEDPC\Downstairs\stgtacr_behavior';
% cd(CurrentDir)
% 
% [~,~,raw] = xlsread('Z:\Dakota\MEDPC\Downstairs\vp-vta-stgtacr_DStrain\vp-vta-stgtacr_mpc2excel.xlsx'); %import the mpc excel data generated by row profile
% [~,~,ratinfo] = xlsread('vp-vta-stgtacr_subj_metadata.xlsx'); %import rat metadata from excel sheet generated manually
...
%% ---- dp vp-vta-stgtacr specific code:
% 
% %% DP adding new behavior analyses & visualization for discrimination criteria
% 
% %calculate inverse of latency- normalized? measure of vigor or
% %responsiveness Shadmehr's Vigor book discusses this
% TrainingData.DSRelLatMeanInv= 1./TrainingData.DSRelLatMean;
% TrainingData.NSRelLatMeanInv= 1./TrainingData.NSRelLatMean;
% 
% %% calculate and plot DS / NS PE Ratio
% 
% TrainingData.RatioDiff= TrainingData.DSRatio./TrainingData.NSRatio;
% % TrainingData.RatioDiff= TrainingData.DS10sResponseProb./TrainingData.NS10sResponseProb;
% 
% %late VTA inhibition
% figure
% selection = TrainingData.ExpType==0 & (strcmp(TrainingData.Projection,'VTA')); 
% i=gramm('x',TrainingData.FinalDay(selection),'y',TrainingData.RatioDiff(selection),'color',TrainingData.Subject(selection))
% i.stat_summary('type','sem','geom','area');
% i.set_names('x','Training Day','y', 'Cue response preference (DS ratio/NS ratio) (respond Y times more to DS; 1= not discriminating)','color','DS(-)','column','RatID');
% i.set_title('Inhib Late VTA discrimination: DS/NS ratio');
% i.axe_property( 'YLim',[0,2]);% max(TrainingData.RatioDiff(selection))]);
% days= TrainingData.FinalDay(selection);
% days= days(~isnan(days));
% i.axe_property( 'XLim',[1, numel(unique(days))]);
% 
% i.geom_hline('yintercept',1) %add a reference line at y= 1 (DS ratio = NS ratio)
% i.geom_hline('yintercept',criteriaDiscrimIndex, 'style', 'r-.'); %add line at criteria threshold
% 
% 
% i.draw()
% 
% i.export( 'file_name','Inhib Late VTA Discrim Index','export_path',SavingDir,'file_type','pdf')
% 
% %% Moving Means of behavior
% %since behavior can vary quite a bit day-to-day, let's get a rolling
% %mean across days for a more accurate baseline 
% 
% % DS & NS ratio
% movWindow= 3; %# sessions to include in rolling mean
% 
% allSubj= unique(TrainingData.RatNum);
% 
% % selection = TrainingData.RatNum==allSubj(thisSubj); 
% 
% %because in one array need to separately movmean() for each subject
% %otherwise will bleed over into other subjects at ends
% for subj= 1:numel(allSubj)%TrainingData.RatNum
%     
%     %find all of the sessions (rows) corresponding to this subject
%     thisSubj= find(TrainingData.RatNum==allSubj(subj));
%     
%     TrainingData.DSRatioMov(thisSubj,:)= (movmean(TrainingData.DSRatio(thisSubj,:), movWindow));
%     TrainingData.NSRatioMov(thisSubj,:)= (movmean(TrainingData.NSRatio(thisSubj,:), movWindow));
% 
%     TrainingData.DSRelLatMeanMov(thisSubj,:)= movmean(TrainingData.DSRelLatMean(thisSubj,:),movWindow);
%     TrainingData.NSRelLatMeanMov(thisSubj,:)=movmean(TrainingData.NSRelLatMean(thisSubj,:),movWindow);
%         
%     TrainingData.DSRelLatMeanInvMov(thisSubj,:)= movmean(TrainingData.DSRelLatMeanInv(thisSubj,:),movWindow);
%     TrainingData.NSRelLatMeanInvMov(thisSubj,:)= movmean(TrainingData.NSRelLatMeanInv(thisSubj,:),movWindow);
% 
%   %for DS/NS, should divide first (already done above) and then movmean()
%   %-- DS/NS is better than subtraction I think because it retains
%   %proportionality and don't have to worry about signs
%     TrainingData.RatioDiffMov(thisSubj,:)= (movmean(TrainingData.RatioDiff(thisSubj,:), movWindow));
%     
%    TrainingData.DeltaRelLatMeanInv(thisSubj,:)= (TrainingData.DSRelLatMeanInv(thisSubj,:)./(TrainingData.NSRelLatMeanInv(thisSubj,:)));
%    TrainingData.DeltaRelLatMeanInvMov(thisSubj,:)= movmean(TrainingData.DeltaRelLatMeanInv(thisSubj,:),movWindow);
%     
%   %for DS-NS, should subtract first and then movmean()
%    TrainingData.DeltaRelLatMean(thisSubj,:)= (TrainingData.DSRelLatMean(thisSubj,:)-(TrainingData.NSRelLatMean(thisSubj,:)));
%    TrainingData.DeltaRelLatMeanMov(thisSubj,:)= movmean(TrainingData.DeltaRelLatMean(thisSubj,:), movWindow);
%    
%    %old subtraction
% %    TrainingData.DeltaRelLatMeanInv(thisSubj,:)= (TrainingData.DSRelLatMeanInv(thisSubj,:)-(TrainingData.NSRelLatMeanInv(thisSubj,:)));
% %    TrainingData.DeltaRelLatMeanInvMov(thisSubj,:)= movmean(TrainingData.DeltaRelLatMeanInv(thisSubj,:),movWindow);
% end
% 
% %% Moving Mean of DS & NS Ratio plot
% figure;
% clear i;
% selection = TrainingData.ExpType==0 & (strcmp(TrainingData.Projection,'VTA')); 
% i(1,1)=gramm('x',TrainingData.FinalDay(selection),'y',TrainingData.DSRatioMov(selection),'color',TrainingData.Subject(selection))
% i(1,1).stat_summary('type','sem','geom','area');
% i(1,1).set_names('x','Training Day','y', 'MovMean DSratio','color','DS(-)','column','RatID');
% i(1,1).set_title('Inhib Late VTA: DS ratio MOVMEAN (3 day window)');
% i(1,1).axe_property( 'YLim',[0,1]);% max(TrainingData.RatioDiff(selection))]);
% days= TrainingData.FinalDay(selection);
% days= days(~isnan(days));
% i(1,1).axe_property( 'XLim',[1, numel(unique(days))]);
% 
% i(1,1).geom_hline('yintercept',criteriaPEratio, 'style', 'r-.'); %add line at criteria threshold
% 
% %NS ratio subplot
% i(2,1)=gramm('x',TrainingData.FinalDay(selection),'y',TrainingData.NSRatioMov(selection),'color',TrainingData.Subject(selection))
% i(2,1).set_line_options( 'styles',{':'});
% 
% i(2,1).stat_summary('type','sem','geom','area');
% i(2,1).set_names('x','Training Day','y', 'MovMean DSratio','color','DS(-)','column','RatID');
% i(2,1).set_title('Inhib Late VTA: NS ratio MOVMEAN (3 day window)');
% i(2,1).axe_property( 'YLim',[0,1]);% max(TrainingData.RatioDiff(selection))]);
% days= TrainingData.FinalDay(selection);
% days= days(~isnan(days));
% i(2,1).axe_property( 'XLim',[1, numel(unique(days))]);
% 
% i(2,1).geom_hline('yintercept',criteriaPEratio, 'style', 'r-.'); %add line at criteria threshold
% 
% 
% i.draw()
% 
% %% Moving mean of DS/NS plot
% 
% figure
% selection = TrainingData.ExpType==0 & (strcmp(TrainingData.Projection,'VTA')); 
% i=gramm('x',TrainingData.FinalDay(selection),'y',TrainingData.RatioDiffMov(selection),'color',TrainingData.Subject(selection))
% i.stat_summary('type','sem','geom','area');
% i.set_names('x','Training Day','y', 'MovMean Cue response preference (DS ratio/NS ratio) (respond Y times more to DS; 1= not discriminating)','color','DS(-)','column','RatID');
% i.set_title('Inhib Late VTA discrimination: DS/NS ratio MOVMEAN (3 day window)');
% i.axe_property( 'YLim',[0,2]);% max(TrainingData.RatioDiff(selection))]);
% days= TrainingData.FinalDay(selection);
% days= days(~isnan(days));
% i.axe_property( 'XLim',[1, numel(unique(days))]);
...
% 
% 
% %% Moving Mean of DS & NS Latency plot
% figure;
% clear i;
% selection = TrainingData.ExpType==0 & (strcmp(TrainingData.Projection,'VTA')); 
% i(1,1)=gramm('x',TrainingData.FinalDay(selection),'y',TrainingData.DSRelLatMeanMov(selection),'color',TrainingData.Subject(selection))
% i(1,1).stat_summary('type','sem','geom','area');
% i(1,1).set_names('x','Training Day','y', 'MovMean latency (s)','color','DS(-)','column','RatID');
% i(1,1).set_title('Inhib Late VTA: DS PE Latency MOVMEAN (3 day window)');
% i(1,1).axe_property( 'YLim',[0,10]);% max(TrainingData.RatioDiff(selection))]);
% days= TrainingData.FinalDay(selection);
% days= days(~isnan(days));
% i(1,1).axe_property( 'XLim',[1, numel(unique(days))]);
% 
% 
% %NS ratio subplot
% i(2,1)=gramm('x',TrainingData.FinalDay(selection),'y',TrainingData.NSRelLatMeanMov(selection),'color',TrainingData.Subject(selection))
% i(2,1).set_line_options( 'styles',{':'});
% 
% i(2,1).stat_summary('type','sem','geom','area');
% i(2,1).set_names('x','Training Day','y', 'MovMean latency (s)','color','NS(-)','column','RatID');
% i(2,1).set_title('Inhib Late VTA: NS PE latency MOVMEAN (3 day window)');
% i(2,1).axe_property( 'YLim',[0,10]);% max(TrainingData.RatioDiff(selection))]);
% days= TrainingData.FinalDay(selection);
% days= days(~isnan(days));
% i(2,1).axe_property( 'XLim',[1, numel(unique(days))]);
% 
...
% i.draw()
% 
% %% Inverse Moving Mean of DS & NS Latency plot
% figure;
% clear i;
% selection = TrainingData.ExpType==0 & (strcmp(TrainingData.Projection,'VTA')); 
% i(1,1)=gramm('x',TrainingData.FinalDay(selection),'y',TrainingData.DSRelLatMeanInvMov(selection),'color',TrainingData.Subject(selection))
% i(1,1).stat_summary('type','sem','geom','area');
% i(1,1).set_names('x','Training Day','y', '1/MovMean latency (s). More positive=faster','color','DS(-)','column','RatID');
% i(1,1).set_title('Inhib Late VTA: Inverse DS PE Latency 1/MOVMEAN (3 day window)');
% i(1,1).axe_property( 'YLim',[0,.5]);% max(TrainingData.RatioDiff(selection))]);
% days= TrainingData.FinalDay(selection);
% days= days(~isnan(days));
% i(1,1).axe_property( 'XLim',[1, numel(unique(days))]);
% 
% 
% %NS ratio subplot
% i(2,1)=gramm('x',TrainingData.FinalDay(selection),'y',TrainingData.NSRelLatMeanInvMov(selection),'color',TrainingData.Subject(selection))
% i(2,1).set_line_options( 'styles',{':'});
% 
% i(2,1).stat_summary('type','sem','geom','area');
% i(2,1).set_names('x','Training Day','y', '1/MovMean latency (s). More positive = faster','color','NS(-)','column','RatID');
% i(2,1).set_title('Inhib Late VTA: NS PE latency MOVMEAN (3 day window)');
% i(2,1).axe_property( 'YLim',[0,.5]);% max(TrainingData.RatioDiff(selection))]);
% days= TrainingData.FinalDay(selection);
% days= days(~isnan(days));
% i(2,1).axe_property( 'XLim',[1, numel(unique(days))]);
% 
...
% 
% 
% %% Plot DS-NS latency to make differences more apparent
% 
% figure;
% clear i;
% selection = TrainingData.ExpType==0 & (strcmp(TrainingData.Projection,'VTA')); 
% i(1,1)=gramm('x',TrainingData.FinalDay(selection),'y',(TrainingData.DeltaRelLatMeanMov(selection)),'color',TrainingData.Subject(selection))
% i(1,1).stat_summary('type','sem','geom','area');
% i(1,1).set_names('x','Training Day','y', 'Latency delta (DS-NS)(s); More negative = respond to DS quicker than NS','color','Latency delta (DS-NS)','column','RatID');
% i(1,1).set_title('Inhib Late VTA: Delta PE Latency (DS-NS) MOVMEAN (3 day window)');
% i(1,1).axe_property( 'YLim',[-5,5]);% max(TrainingData.RatioDiff(selection))]);
% days= TrainingData.FinalDay(selection);
% days= days(~isnan(days));
% i(1,1).axe_property( 'XLim',[1, numel(unique(days))]);
% 
% i(1,1).geom_hline('yintercept',0); %add line at 0 threshold
% 
% 
% i.draw();
% %% Plot Inverse DS/NS latency to make differences more apparent
% 
% figure;
% clear i;
% selection = TrainingData.ExpType==0 & (strcmp(TrainingData.Projection,'VTA')); 
% i(1,1)=gramm('x',TrainingData.FinalDay(selection),'y',(TrainingData.DeltaRelLatMeanInvMov(selection)),'color',TrainingData.Subject(selection))
% i(1,1).stat_summary('type','sem','geom','area');
% i(1,1).set_names('x','Training Day','y', 'Inverse latency delta (1/DS / 1/NS))(s). More positive = respond Y times quicker to DS than NS. 1=no latency difference. <1= NS quicker','color','Latency delta (DS-NS)','column','RatID');
% i(1,1).set_title('Inhib Late VTA: Inverse Delta PE Latency (1/DS / 1/NS) MOVMEAN (3 day window)');
% i(1,1).axe_property( 'YLim',[.5,2]);% max(TrainingData.RatioDiff(selection))]);
% days= TrainingData.FinalDay(selection);
% days= days(~isnan(days));
% i(1,1).axe_property( 'XLim',[1, numel(unique(days))]);
% 
% i(1,1).geom_hline('yintercept',1); %add line at 0 threshold
% 
% 
% i.draw();
% 
...
% % i(1,1)=gramm('x',TrainingData.FinalDay(selection),'y',(TrainingData.DeltaRelLatMeanInvMov(selection)),'color',TrainingData.Subject(selection))
% % i(1,1).stat_summary('type','sem','geom','area');
% % i(1,1).set_names('x','Training Day','y', 'Inverse latency delta (1/(DS-NS))(s). More positive = respond quicker to DS than NS. 0=no latency difference. Negative= NS quicker','color','Latency delta (DS-NS)','column','RatID');
% % i(1,1).set_title('Inhib Late VTA: Inverse Delta PE Latency (1/DS-NS) MOVMEAN (3 day window)');
% % i(1,1).axe_property( 'YLim',[0,.5]);% max(TrainingData.RatioDiff(selection))]);
% % days= TrainingData.FinalDay(selection);
% % days= days(~isnan(days));
% % i(1,1).axe_property( 'XLim',[1, numel(unique(days))]);
...
% 
% 
% %% compare with early data
% 
% figure
% selection = TrainingData.ExpType==0 & (strcmp(TrainingData.Projection,'VTA')); 
% i=gramm('x',TrainingData.EarlyDay(selection),'y',TrainingData.RatioDiffMov(selection),'color',TrainingData.Subject(selection))
% i.stat_summary('type','sem','geom','area');
% i.set_names('x','Training Day','y', 'MovMean Cue response preference (DS ratio/NS ratio) (respond Y times more to DS; 1= not discriminating)','color','DS(-)','column','RatID');
% i.set_title('Inhib Late VTA discrimination: DS/NS ratio');
% i.axe_property( 'YLim',[0,2]);% max(TrainingData.RatioDiff(selection))]);
...
% 
% i.geom_hline('yintercept',1) %add a reference line at y= 1 (DS ratio = NS ratio)
% i.geom_hline('yintercept',criteriaDiscrimIndex); %add line at criteria threshold
% 
% i.draw()
% 
% %% DS - NS ratio
...
% % i=gramm('x',TrainingData.FinalDay(selection),'y',TrainingData.RatioDiff(selection),'color',TrainingData.Subject(selection))
% % i.stat_summary('type','sem','geom','area');
% % i.set_names('x','Training Day','y', 'Response Probability (10s) DS-NS (+1 = respond exclusively to DS,0= not discriminating, -1 =respond exclusively to NS)','color','DS(-)','column','RatID');
% % i.set_title('Inhib Late VTA discrimination: DS-NS ratio');
...
% % i.axe_property( 'XLim',[1, numel(unique(days))]);
% % 
% % i.geom_hline('yintercept',0) %add a reference line at y= 0 (DS ratio = NS ratio)
% % 
% % i.draw()
% % 
% % i.export( 'file_name','Inhib Late VTA Discrim Index B','export_path',SavingDir,'file_type','pdf')
% 
% 
% %% Total Port Entries
% 
% % %strip() empty space in totalPE values and convert to numbers for plotting
% % %todo: must be a more efficient way, strip early in code for all vars?
% % TrainingData.TotalPE = str2double(strip(string(TrainingData.TotalPE)));
% % 
% % %late VTA inhibition
% % figure
% % selection = TrainingData.ExpType==0 & (strcmp(TrainingData.Projection,'VTA')); 
% % i=gramm('x',TrainingData.FinalDay(selection),'y',TrainingData.TotalPE(selection),'color',TrainingData.Subject(selection))
% % i.stat_summary('type','sem','geom','area');
% % i.set_names('x','Training Day','y', 'Total PE count)','color','DS(-)','column','RatID');
% % i.set_title('Inhib Late VTA: Total PE Count');
% % i.axe_property( 'YLim',[0, 1000]);%max(TrainingData.TotalPE(selection))]);
% % days= TrainingData.FinalDay(selection)
% % days= days(~isnan(days));
% % i.axe_property( 'XLim',[1, numel(unique(days))]);
% % 
% % i.draw()
% % 
% % i.export( 'file_name','Inhib Late VTA Total PE','export_path',SavingDir,'file_type','pdf')
% 
% 
% %% PE selectivity "index" : Num DS PE / Total PE
% % % % TODO: MPC calculated DS & NS numbers probably not correct, may need
% % % manual calculation 
% % %could also do DS port time / total port time
% % 
% % %first strip and convert to number
% % TrainingData.DSPE = str2double(strip(string(TrainingData.DSPE)));
% % 
% % % calculate discrimination index
% % TrainingData.DiscrimInd= TrainingData.DSPE./TrainingData.TotalPE;
% % 
% % figure
% % selection = TrainingData.ExpType==0 & (strcmp(TrainingData.Projection,'VTA')); 
% % i=gramm('x',TrainingData.FinalDay(selection),'y',TrainingData.DiscrimInd(selection),'color',TrainingData.Subject(selection))
% % i.stat_summary('type','sem','geom','area');
% % i.set_names('x','Training Day','y', 'PE discrimination index (0= PE only outside of DS, 1= PE only during DS)','color','DS(-)','column','RatID');
% % i.set_title('Inhib Late VTA: Discrimination index (Num DS PE / Num Total PE)');
% % i.axe_property( 'YLim',[0, .2]);%max(TrainingData.TotalPE(selection))]);
% % days= TrainingData.FinalDay(selection);
% % days= days(~isnan(days));
% % i.axe_property( 'XLim',[1, numel(unique(days))]);
% % 
% % i.draw()
% % 
% % i.export( 'file_name','Inhib Late VTA Seeking Specificity','export_path',SavingDir,'file_type','pdf')
% 
% 
% %% figure for each subj w subplots for criteria
% allSubj= unique(TrainingData.RatNum)
% for thisSubj=1:numel(allSubj) %could be more efficient todo
%     figure
%     selection = TrainingData.RatNum==allSubj(thisSubj); 
%     i(1,1)=gramm('x',TrainingData.FinalDay(selection),'y',TrainingData.DS10sResponseProb(selection),'color',TrainingData.Subject(selection))
%     i(1,1).stat_summary('type','sem','geom','area');
%     i(1,1).set_names('x','Training Day','y', 'Response Probability (10s)','color','DS(-)','column','RatID');
%     i(1,1).set_title(strcat(num2str(thisSubj),'Inhib Late VTA Response Probability'));
%     i(1,1).axe_property( 'YLim',[0 1]);
%     days= TrainingData.FinalDay(selection);
%     days= days(~isnan(days));
%     i(1,1).axe_property( 'XLim',[1, numel(unique(days))]);i(1,1).draw()
% 
%     i(1,1).geom_hline('yintercept',criteriaPEratio); %add line at criteria threshold
% 
% 
%     i(1,1).update('x',TrainingData.FinalDay(selection),'y',TrainingData.NS10sResponseProb(selection),'color',TrainingData.Subject(selection))
%     i(1,1).stat_summary('type','sem','geom','area');
%     i(1,1).set_names('x','Training Day','y','Response Probability (10s)');
%     i(1,1).set_line_options( 'styles',{':'});
%     i(1,1).axe_property( 'YLim',[0 1]);
%     days= TrainingData.FinalDay(selection);
%     days= days(~isnan(days));
%     i(1,1).axe_property( 'XLim',[1, numel(unique(days))]);i(1,1).no_legend()
% %     i(1,1).draw()
%     
%     %subplot for discrim criteria
%     i(2,1)=gramm('x',TrainingData.FinalDay(selection),'y',TrainingData.RatioDiff(selection),'color',TrainingData.Subject(selection))
%     i(2,1).stat_summary('type','sem','geom','area');
%     i(2,1).set_names('x','Training Day','y', 'Cue response preference (DS ratio/NS ratio) (respond Y times more to DS; 1= not discriminating)','color','DS(-)','column','RatID');
%     i(2,1).set_title(strcat(num2str(thisSubj),'Inhib Late VTA discrim DS/NS ratio'));
%     i(2,1).axe_property( 'YLim',[0, max(TrainingData.RatioDiff(selection))]);
%     days= TrainingData.FinalDay(selection);
%     days= days(~isnan(days));
%     i(2,1).axe_property( 'XLim',[1, numel(unique(days))]);
% 
%     i(2,1).geom_hline('yintercept',1) %add a reference line at y= 1 (DS ratio = NS ratio)
%     i(2,1).geom_hline('yintercept',criteriaDiscrimIndex); %add line at criteria threshold
% 
% %     i(2,1).draw()
%     
%     i.draw()
%     
% %     i.export( 'file_name','subInhib Late VTA Response Probability','export_path',SavingDir,'file_type','pdf')
